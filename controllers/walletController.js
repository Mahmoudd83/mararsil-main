  ...
  try {
    ...
    const response = await axios.post(
      "https://api.moyasar.com/v1/payments",
      {
        ...
        callback_url:
          "https://c940b43e3973.ngrok-free.app/api/wallet/webhook/moyasar",
      },
      {
        auth: {
...
exports.uplaodBankreceiptImage = uploadSingleImage("bankReceipt");

exports.ResizeImage = asyncHandler(async (req, res, next) => {
  const filename = `bankReceipt-${uuidv4()}-${Date.now()}.jpeg`;
  if (req.file) {
    await sharp(req.file.buffer)
      ...
      .toFormat("jpeg")
      .jpeg({ quality: 95 })
      .toFile(`uploads/bankReceipt/${filename}`);
    req.body.bankReceipt = filename;
  }
  next();
});
...
exports.RechargeWalletbyBank = asyncHandler(async (req, res, next) => {
  try {
    const { amount,bankReceipt } = req.body;

    const transaction = new Transaction({
      customerId: req.customer._id,
      amount,
      description:"Recharge wallet by bank",
      type: "credit",
