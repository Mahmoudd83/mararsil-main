...
class ShopifyPlatform {
  ...
  async getOrders(params = {}) {
    ...
    async registerWebhook({ topic, address, format = "json" }) {
    try {
      const response = await axios.post(
      ...
      if (!response.data.webhook) {
        throw new Error("Shopify لم ترجع Webhook");
      }

      console.log(" Webhook اتسجل بنجاح:", response.data.webhook);
      return response.data.webhook;
    } catch (error) {
      console.error(
        "خطأ أثناء تسجيل Webhook:",
        error.response?.data || error.message
      );
      throw new Error("فشل تسجيل Webhook");
    }
  }
  ...
  try {
    const response = await axios.post(
      `https://${this.storeName}.myshopify.com/admin/api/${this.apiVersion}/webhooks.json`,
      {
        webhook: {
          topic: 'orders/create',
          address: 'https://46a9f841a929.ngrok-free.app/api/shopify/webhook',
          format: 'json'
        }
      },
      {
